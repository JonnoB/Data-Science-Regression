...
```
#Exectutive Summary
The report reviews 5 different models and selects a model without any interaction, the modal has an adjusted $R^2$ of 84% and shows a increase of 1.8mpg when driving a manual car, however this result does not appear to be significant at the 95% confidence level.
...
mpg.norm <- data.frame(Type = c("All Cars", "Manual", "Automatic"), P.values = round(P.values, 4)); rownames(mpg.norm) <-NULL
```
...
Reviewing the modals, mod2 can be rejected as it is essentially the same as the two means that were explored at the beginning of this document. Although the Mod1 varients have extremly high $R^2$ values due to the interaction terms, these interaction terms make interpreting the model more difficult. 
where there are interaction ters it is possible to do a montecarlo analysis on the Automatic/Manual variable but this needs to be carefullt controlled otherwise unrealistic results can be obtained (see appendix for an example)
Due to these considerations the Modal "mod0step" was chosen to be used in the final evaluation.

##Results

```{r, results='asis'}
print(xtable(as.data.frame(summary(mod0step)$coefficients)), type= pub.type)
```
A plot of the residuals (see appendix) shows that the residuals are reasonaby normally distributed.

#Conclusions
In conclusion it appears that in 1974 manual cars were `r mod0step$coefficients["am1"]` mpg more fuel efficient than automatic cars when controlling for cylinders, horsepower and car weight. However when controlling for these variables the p value of the "am" variable is not significant at the 0.95 confidence level meaning ***there doesn't seem to be a significant difference between the two car types***. Clearly there appears to be significant interaction between variables exploring this further could provide more difintive/nuanced results, an example is that although not shown in this report controlling for rear axel ratio (drat) appeared to reverse the sign on the fuel efficiency of the car, this is definately worth investigating further
...
```{r}
par(mfrow= c(1,2))
plot(data$mpg,resid(mod0step), main = "residual plot of the fitted model", xlab="mpg", ylab="Modal Residuals")
abline(0, 0)

qqnorm(resid(mod0step))
qqline(resid(mod0step))
```
